...
class BlenderMCPServer:
    ...
    def _handle_client(self, client):
        ...
        Fix (Conjure3D): The original pattern called client.sendall() inside a
        bpy.app.timers callback running on Blender's main thread while this
        handler thread had already looped back to client.recv() — concurrent
        access to the same socket on Windows causes WinError 10053
        (WSAECONNABORTED). Fixed with threading.Event: main thread executes the
        command and stores the serialised result, then sets the event; this
        handler thread waits on the event and sends the response itself,
        so only one thread ever touches the socket at a time.
        """
        ...
        try:
            while self.running:
                try:
                    ...
                    def execute_wrapper(cmd=command, ev=done, holder=result_holder):
                        try:
                            holder[0] = json.dumps(self.execute_command(cmd))
                        except Exception as e:
                            print(f"Error executing command: {str(e)}")
                            traceback.print_exc()
                            holder[0] = json.dumps({"status": "error", "message": str(e)})
                        finally:
                            ev.set()
                        return None
                    ...
                    if not done.wait(timeout=180.0):
                        print("Command timed out waiting for main thread (180 s)")
                        break

                    try:
                        client.sendall(result_holder[0].encode('utf-8'))
                    except Exception as e:
